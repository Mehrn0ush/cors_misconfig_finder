 ...
 */
 ...
 async fn main() {
     ...
     let custom_headers = matches.get_one::<String>("custom_headers");
     let cookie = matches.get_one::<String>("cookie");
     let rate_limit = matches.get_one::<u64>("rate_limit").copied();
     let method = matches.get_one::<String>("method").map(|s| s.as_str()).unwrap_or("GET");
     ...
     scan(url, custom_headers.map(|s| s.as_str()), cookie.map(|s| s.as_str()), proxy.map(|s| s.as_str()), output.map(|s| s.as_str()), silent, no_color, rate_limit, method, thirdparty, invalid_origin).await;
 }
